...
let () =
  ...
  Out_channel.with_open_text "vm/dune" (fun dunech ->
      ...
      dprintfn "(cram (deps ../../../src/Driver.exe ../../../runtime/Std.i ../../../virtual_machine/lama.exe %s))\n"
        (String.concat " " (List.concat_map (fun s ->
            [sprintf "../../../stdlib/x64/%s.i" s
            ;sprintf "../../../stdlib/x64/%s.bc" s
            ])
            stdlib));
      ...
      for i = 0 to count - 1 do
        ...
        let cram_printfn fmt =
          Format.kasprintf (Printf.bprintf cram_buf "%s\n") fmt
        in
        let cram_file = Printf.sprintf "vm/test%02d.t" i in
        let lama_file = Printf.sprintf "test%02d.lama" i in

        if Sys.file_exists lama_file then (
          cram_printfn
            "  $ ../../../src/Driver.exe -runtime ../../../runtime -I ../../../runtime -I ../../../stdlib/x64 -b ../test%02d.lama"
            i;
          cram_printfn
            "  $ ../../../virtual_machine/lama.exe -I ../../../stdlib/x64 test%02d.bc"
            i;
